...
let TelemetryInterceptor = class TelemetryInterceptor {
    ...
    intercept(context, next) {
        ...
        const metricsConfig = this.reflector.getAllAndOverride(telemetry_decorator_1.TELEMETRY_METRICS_KEY, [context.getHandler(), context.getClass()]);
        ...
        if (metricsConfig?.counter && this.telemetryService) {
            this.telemetryService.incrementCounter(metricsConfig.counter, metricsConfig.attributes || {});
        }
        return next.handle().pipe((0, rxjs_1.tap)((result) => {
            const duration = Date.now() - startTime;
            if (metricsConfig?.histogram && this.telemetryService) {
                this.telemetryService.recordHistogram(metricsConfig.histogram, duration, { ...metricsConfig.attributes, status: 'success' });
            }
            ...
            const duration = Date.now() - startTime;
            if (metricsConfig?.histogram && this.telemetryService) {
                this.telemetryService.recordHistogram(metricsConfig.histogram, duration, { ...metricsConfig.attributes, status: 'error' });
            }
